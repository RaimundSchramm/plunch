class MealLogEntriesController < ApplicationController
  ...
  def index
    @meal_log_entries = MealLogEntry.all
  end
  ...
  def new
    @meal_log_entry = MealLogEntry.new
  end
  ...
  def create
    @meal_log_entry = MealLogEntry.new(meal_log_entry_params)

    respond_to do |format|
      if @meal_log_entry.save
        format.html { redirect_to @meal_log_entry, notice: 'Meal log entry was successfully created.' }
  ...
  def update
    respond_to do |format|
      if @meal_log_entry.update(meal_log_entry_params)
        format.html { redirect_to @meal_log_entry, notice: 'Meal log entry was successfully updated.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to meal_log_entries_url, notice: 'Meal log entry was successfully destroyed.' }
  ...
  private
    ...
    def set_meal_log_entry
      @meal_log_entry = MealLogEntry.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def meal_log_entry_params
      params.require(:meal_log_entry).permit(:eaten_on, :meal)
    end
end
